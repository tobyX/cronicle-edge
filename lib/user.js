...
module.exports = Class.create({
	...
	create_ad_user: async function (user, password, domain, args) {
		var self = this;
		return new Promise((resolve, reject) => {
			const config = { url: 'ldaps://' + domain,
							baseDN: 'OU=DE,DC=internal,DC=vodafone,DC=com',
							username: 'VF-ROOT\\' + user,
							password: password,
							logging: pino({level: 'trace'}), 
							tlsOptions: { 'rejectUnauthorized': false } }
			let ad = new ActiveDirectory(config);
			ad.findUser(user, function(err, ldap_user) {
				if (err || ! ldap_user) {
					resolve(false);
				}
				else {
					self.logDebug(6, 'Got user data from AD: ' + JSON.stringify(ldap_user));
			
					let new_user = {}
					new_user.username = ldap_user.sAMAccountName;
					new_user.email = ldap_user.mail;
					new_user.full_name = ldap_user.displayName;
					new_user.password = Tools.generateUniqueID(64, ldap_user.pwdLastSet).substring(0, 12);

					var path = 'users/' + self.normalizeUsername(new_user.username);
					...
					new_user.ext_auth = true;
					new_user.privileges = Tools.copyHash(self.config.get('default_privileges') || {});
	...
	do_ad_auth: function (user, password, domain) {
		return new Promise((resolve, reject) => {
			...
			let ad = new ActiveDirectory({ url: ('ldaps://' + domain), logging: log, tlsOptions: tlsOptions});
	...
	api_login: async function (args, callback) {
		...
		var ad_user = 'VF-ROOT\\' + params.username;

		if (!user) {
			let isValidAD = await self.do_ad_auth(ad_user, params.password, ad_domain);

			if (isValidAD) {
				user = await self.create_ad_user(params.username, params.password, ad_domain, args, callback);
				self.logDebug(3, "User created: " + JSON.stringify(user));
				if (! user) {
